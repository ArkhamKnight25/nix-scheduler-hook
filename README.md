...
- `remote-building`: Build over SSH rather than as part of the job script. Avoids copying the entire derivation closure to the remote. The job script just becomes a reservation system and will exit once the outputs exist. The SSH user must be a trusted user on the remotes. Three consequences worth knowing: the build runs under the node's own daemon rather than inside the job script, so the allocation's cgroup, cpuset and memory limits do not apply to it, and the job script's `--option system-features` no longer applies either -- the build nodes' own `system-features` must list everything your derivations require, including the `nsh` marker. The reservation's exit status is advisory, since by the time it exits the build has already finished. And the setting is ignored for whole-graph builds (`--store nsh://`), which have no input closure to hand the node's daemon and so must keep realising the derivation from the job script.
